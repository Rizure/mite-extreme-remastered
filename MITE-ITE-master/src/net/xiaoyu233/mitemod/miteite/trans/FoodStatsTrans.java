      ...
      if (!par1EntityPlayer.isGhost() && !par1EntityPlayer.isZevimrgvInTournament()) {
         if (!par1EntityPlayer.isDead && !(par1EntityPlayer.getHealth() <= 0.0F)) {
            ...
            if (this.player.isStarving()) {
               ...
               heal_base *= par1EntityPlayer.isMalnourished() ? 0.25F : 1.0F;
               heal_base *= par1EntityPlayer.inBed() ? 4.0F : 1.0F;
               heal_base *= EnchantmentManager.getRegenerationModifier(this.player);
