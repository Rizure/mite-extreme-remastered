...
public class ServerNetworkManagerTrans extends NetworkManagerTrans {
   ...
   public void handleUpdateForgingTableModeState(SPacketUpdateForgingTableModeState packet){
      // 此方法已在主线程执行（由 processPacket 调度）
      bdd world = this.h.f; // 假设 this.h 是 Minecraft，.f 是 WorldClient
      if (world == null) return;

      // 更新 TileEntity 数据
      TileEntity te = world.getBlockTileEntity(packet.getX(), packet.getY(), packet.getZ());
      if (te instanceof TileEntityForgingTable) {
         ((TileEntityForgingTable) te).setForging_mode(packet.getState());
      }

      awe openingGUI = this.h.n;
      if (openingGUI instanceof GuiForgingTable) {
         GuiForgingTable gui = (GuiForgingTable) openingGUI;
         // 检查 GUI 对应的方块位置是否与包中的位置一致
         if (gui.getX() == packet.getX() &&
             gui.getY() == packet.getY() &&
             gui.getZ() == packet.getZ()
         ) {
            gui.enableButton();
            gui.setBlockMode(packet.getState());
         }
      }
   }
