...
public class ForgingTableSlots extends InventorySubcontainer {
   ...
   public ForgingTableSlots(IInventory forgingTable) {
      ...
      this.axe = new Slot(forgingTable, this.getAxeSlotIndex(), 137, 62) {
         public boolean isItemValid(ItemStack par1ItemStack) {
            return par1ItemStack.getItem() instanceof ItemBattleAxe;
         }
      };
   ...
   public boolean hasUniversal_withEffecting() {
      ...
      for (Slot current : currentMaterials) {
         if (current.getStack() != null && ItemStack.areItemStacksEqual(Items.enhanceStoneUniversal.getItemStackForStatsIcon(), current.getStack(), true, false, false, true)) {
   ...
   public ItemStack getTemplates() {
      ItemStack item = null;
      List<Slot> currentMaterials = Lists.newArrayList(this.upLeft, this.upRight, this.left, this.right, this.downLeft, this.downRight);
      for (Slot current : currentMaterials) {
         if (current.getStack() != null && current.getStack().getItem() instanceof ItemForgingTemplates) {
            item = current.getStack();
            int resultSize = current.getStack().stackSize - 1;
            if (resultSize > 0) {
               current.getStack().setStackSize(resultSize);
            } else {
   ...
   public ForgingRecipe getRecipeFromTool(@Nonnull ItemStack toolStack) {
      return ForgingTableRecipes.getRecipe(
              ...
              this.container.getTileentity().getForging_mode()
      );
   }
