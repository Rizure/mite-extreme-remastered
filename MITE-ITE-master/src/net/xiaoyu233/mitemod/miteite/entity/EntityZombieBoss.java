...
//    private Enchantment [] nonLevelsBookList = new Enchantment[] {Enchantments.enchantmentFixed, Enchantments.enchantmentChain, Enchantments.EMERGENCY};
   ...
   private void initDroppingEnchantmentList() {
      for (int i = 0; i < enchantmentsList.length; i++) {
         if (enchantmentsList[i] != null) {
            if (enchantmentsList[i].getNumLevels() != 1) {
               enhanceSpecialBookList.add(enchantmentsList[i]);
            } else {
               nonLevelsBookList.add(enchantmentsList[i]);
            }
         }
      }
   }
   ...
   public EntityZombieBoss(World par1World) {
      super(par1World);
      this.initDroppingEnchantmentList();
   }

   protected void addRandomEquipment() {
      this.setCurrentItemOrArmor(0, new ItemStack(Items.warHammerVibranium, 1).randomizeForMob(this, true));
      this.setCurrentItemOrArmor(1, new ItemStack(Items.helmetVibranium, 1).randomizeForMob(this, true));
      this.setCurrentItemOrArmor(2, new ItemStack(Items.cuirassVibranium, 1).randomizeForMob(this, true));
      this.setCurrentItemOrArmor(3, new ItemStack(Items.leggingsVibranium, 1).randomizeForMob(this, true));
      this.setCurrentItemOrArmor(4, new ItemStack(Items.bootsVibranium, 1).randomizeForMob(this, true));
   }
...
//    }
   ...
   protected void dropFewItems(boolean recently_hit_by_player, DamageSource damage_source) {
      if (recently_hit_by_player) {
         ...
         dropEnchantment = this.nonLevelsBookList.get(this.rand.nextInt(this.nonLevelsBookList.size()));
         ...
         dropEnchantment = this.enhanceSpecialBookList.get(this.rand.nextInt(this.enhanceSpecialBookList.size()));
         stack = Item.enchantedBook.getEnchantedItemStack(new EnchantmentInstance(dropEnchantment, dropEnchantment.getNumLevelsForVibranium()));
         this.dropItemStack(stack);
         if (!this.isFinal()) {
            this.dropItem(Items.cracked_key);
            this.dropItemStack(new ItemStack(Items.itemEnhanceGem3, 1, GemModifierTypes.fetchingSubWithTier(this.rand, 1)));
         } else {
            for (int i = 0; i < 3; i++) {
               this.dropItem(Items.voucherCore);
            }
            this.dropItemStack(new ItemStack(Items.itemEnhanceGem5, 1, GemModifierTypes.fetchingSubWithTier(this.rand, 1)));
            this.dropItemStack(new ItemStack(Items.itemEnhanceGem3, 1, GemModifierTypes.fetchingSubWithTier(this.rand, 2)));
         }
      }
   }
   ...
   protected void applyEntityAttributes() {
      ...
      this.setEntityAttribute(GenericAttributes.maxHealth, Math.min(1000, 400 * Constant.getBossMobModifier("Health", day)));
