...
public class ItemGAFood extends ItemFood {
   ...
   protected void onEaten(ItemStack par1ItemStack, World par2World, EntityPlayer par3EntityPlayer) {
      ...
      if (Objects.equals(this.tag, "suspicious_stew")) {
         float health_delta = par3EntityPlayer.getMaxHealth() - par3EntityPlayer.getHealth();
         if(health_delta < 0F){
            health_delta = 0F;
         }
         int outcome = (int)(health_delta * 4.0F) + 1;
         par3EntityPlayer.addPotionEffect(new MobEffect(MobEffectList.regeneration.id, outcome * 3, 4));
      }
      if (Objects.equals(this.tag, "spicy_strip")) {
         par3EntityPlayer.triggerAchievement(Achievements.spicyStrip);
         float healthDelta = Math.min(6.0F, par3EntityPlayer.getHealth() - 1.0F);
         if(healthDelta > 0F){
            par3EntityPlayer.attackEntityFrom(new Damage(DamageSource.poison, healthDelta));
            par3EntityPlayer.addPotionEffect(new MobEffect(MobEffectList.regeneration.id, 20 + (int)(healthDelta * 50.0F), 0));
         }
      }
      ...
      if (Objects.equals(this.tag, "chikitan")) {
         ...
         if (outcome == 0) {
            ...
            int level = par3EntityPlayer.getEnhanceLevel();
            par3EntityPlayer.sendChatToPlayer(ChatMessage.createFromText("当前修为等级: " + level));
